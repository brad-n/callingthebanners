app.factory('Api', ['Restangular', '$rootScope', '$q', '$location', '$http',
	function (Rest, $rootScope, $q, $location, $http) {
		...
		return {
			...
			/* Login / Account specific details */
			getAccountDetails: function(opts){
				return Rest.all('account/checkauth/').get('');
			},
			
			createAccount: function(opts){
				return Rest.all('account/create/').post(opts);
			},
			accountLogin: function(opts){
				return Rest.all('account/signin/').post(opts);
			},
			accountLogout: function(){
				return Rest.all('account/signout/').get('');
			},
