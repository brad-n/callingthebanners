...
$app = new \Slim\App;
$GLOBALS['db'] = mysqli_connect('localhost', $cfg['db_user'], $cfg['db_pass'], 'callingthebanners');
...
$app->get('/account/checkauth/', function (Request $request, Response $response) {
    ...
    //are we authed?
    if($U->isAuthed()){
        //get the user to return if we are.
        $user = $U->returnUser();
    }
...
$app->get('/account/signout/', function (Request $request, Response $response) {
    
    //destory our session to log the user out
    session_destroy();
    
    return return_json(array('msg'=>'Logged out!'));
...
$app->post('/account/signin/', function (Request $request, Response $response) {
    ...
    $U = new User($GLOBALS['db']);
    if($U->authUser($body->user_email, $body->password)){
         $res['user'] = $U->returnUser();
    }else{
...
$app->post('/account/create/', function (Request $request, Response $response) {
    
    $body = json_decode($request->getBody());
    
    $user = false;
    
    if(!empty($body->user_email) && !empty($body->password)){
        
        $U = new User($GLOBALS['db']);
        $res['user'] = $U->createUser($body->user_name, $body->user_email, $body->password);
    }
...
$app->get('/groups/list', function(Request $request, Response $response){

    $U = new User($GLOBALS['db']);
    
    if(!$U->isAuthed()){
        return return_json(array('result'=>0, 'auth'=>0, 'message' => 'Unauthorized User'));
    }
...
$app->post('/send-mail/', function(Request $request, Response $response){
	...
	if(!empty($email) && !empty($message)){
		...
		$mg->messages()->send('mg.isleofravens.com', [
				...
				'subject' => 'CtB Contact Sumbission',
				'text'    => $message
		]);
